...
class SparkSqlAstBuilder extends AstBuilder {
  ...
  override def visitExplain(ctx: ExplainContext): LogicalPlan = withOrigin(ctx) {
    ...
    val statement = plan(
      if (ctx.statement != null) ctx.statement
      else ctx.setConfigurationStatement)
    if (statement == null) {
  ...
  /**
   * Fail SET ROLE - exception to the failNativeCommand rule,
   *   because it needs to be handled before other SET commands.
   */
  override def visitFailSetRole(
      ctx: FailSetRoleContext): LogicalPlan = withOrigin(ctx) {
